    ...
    render() {
        ...
        return (
            <View style={{backgroundColor: '#f5f5f5', flex: 1}}>
                {console.log("79:"+JSON.stringify(MeMarkList.markList))}
                ...
                >
                    <View style={[styles.contentContainer, {height: MeMarkList.maxHeight + 40}]}>
                        {MeMarkList.markList.map((feed, i) => {
                            console.log("98:"+JSON.stringify(feed))
                            return (
                                <HomeItem
                                    key={`${feed.id}-${i}`}
                                    feed={feed}
                                    data={MeMarkList.cachedArray[i]}
                                    onPress={() => this._onPressCell(feed)}
                                />
                            )
                        })}
                        <View style={[styles.loadingContainer, {top: MeMarkList.maxHeight+50}]}>
                            <ActivityIndicator />
