...
import {
    fetchFeedList
} from '../../explore/ServOfferListActions';
    ...
    render() {
        ...
        return (
            <View style={{backgroundColor: '#f5f5f5', flex: 1}}>
                {console.log("79:"+JSON.stringify(MeOfferList.meofferList))}
                ...
                >
                    <View style={[styles.contentContainer, {height: MeOfferList.maxHeight + 40}]}>
                        {MeOfferList.meofferList.map((feed, i) => {
                            console.log("98:"+JSON.stringify(feed))
                            return (
                                <HomeItem
                                    key={`${feed.id}-${i}`}
                                    feed={feed}
                                    data={MeOfferList.cachedArray[i]}
                                    onPress={() => this._onPressCell(feed)}
                                />
                            )
                        })}
                        <View style={[styles.loadingContainer, {top: MeOfferList.maxHeight+50}]}>
                            <ActivityIndicator />
...
const HomeItem = ({
    ...
    let serv_image = feed.serv_images?{uri:feed.serv_images.split(',')[0]}:require('../../resource/qk_nav_default.png')
    console.log("135")
    return (
        <TouchableOpacity
            activeOpacity={0.75}
        ...
        >
            ...
            <View style={{
                ...
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    ...
                    <Text style={{fontSize: 12, color: 'gray', marginLeft: 2}}>{feed.favorites_count?feed.favorites_count:0}</Text>
                </View>
            </View>
